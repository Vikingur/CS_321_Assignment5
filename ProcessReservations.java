      ...
      //Iterate across all reservations associated with the current date to flag as late, must-pay, cancel, etc.
		for(int i = 0; i < Calendar.getCurrentDate().getNumReservations(); i++){
			...
			if (currentReservation.getStatus() == 1){
				...
				if(Calendar.getCurrentDate() >= currentReservation.getStartDate()){
         ...
         //For reservations who have been checked-in
            ...
            if(currentReservation.getEndDate() < Calendar.getCurrentDate()){
               currentReservation.setEndDate(Calendar.getCurrentDate() + 1);
               Calendar.getCurrentDate().removeReservation(currentReservation.getID());
               i--;
            }
         }
		}
