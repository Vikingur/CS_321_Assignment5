...
public class CreateReservation
{
	public static String makeReservation(String[] inReservationInfo){
      ...
      if(reservation.getStartDate() > 31 || reservation.getStartDate() < 1){
         return "Invalid start date. Unable to create reservation for "+customer.getName()+".\n";
      }
      
      reservation.setEndDate(Integer.parseInt(inReservationInfo[4]));
      if(reservation.getEndDate() > 31 || reservation.getEndDate() < 1){
         return "Invalid end date. Unable to create reservation for "+customer.getName()+".\n";
      }
      
      if(reservation.getEndDate() <= reservation.getStartDate()){
         return "Specified end date is before start date. Unable to create reservation for "+customer.getName()+".\n";
      }
      
      reservation.setRoomType(Integer.parseInt(inReservationInfo[5]));
      if(reservation.getRoomType() > 2 || reservation.getRoomType() < 1){
         return "Specified room type is not valid. Unable to create reservation for "+customer.getName()+".\n";
      }
      if(Calendar.checkRoomAvailability(reservation.getRoomType(), reservation.getEndDate()) == false){
         return "No available rooms of that type in this time frame. Unable to create reservation for "+customer.getName()+".\n";
      }
      ...
      if(reservation.getNumOccupants() > 4 || reservation.getNumOccupants() < 1){
         return "Invalid number of occupants. Unable to create reservation for "+customer.getName()+".\n";
      }
      reservation.setGuaranteed(Integer.parseInt(inReservationInfo[7]));
      if(reservation.getGuaranteed() > 1 || reservation.getGuaranteed() < 0){
         return "Invalid guarantee status. Unable to create reservation for "+customer.getName()+".\n";
      }
      ...
      if(Integer.parseInt(inReservationInfo[7]) == 1){
         ...
         if(BankSystem.validateCard(customer.getCCNumber(), customer.getCCType(), customer.getCCExpiration()) == false){
            return "Invalid credit card. Unable to create reservation for "+customer.getName()+".\n";
         }
      }
      ...
      return "Successfully created reservation, "+ customer.getName() +". Your Customer ID is: " + CID + " and your Reservation ID is: " + RID + ".\n";
   }
}